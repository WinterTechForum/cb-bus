...
object RouteWithTimesTest extends ZIOSpecDefault {
  val routeLeg1 =
    RouteLeg(
      ...
      RtaSouthbound.componentName).getOrElse(???)
    ...
    suite("RouteWithTimes")(
      suite("nextBefore") (
        test("nextBefore(routeLeg1.ends) gives None"):
          assertTrue:
            routeWithTimes.nextBefore:
              RouteSegment.fromRouteLeg(routeLeg1.ends.getOrElse(???))
            .isEmpty
        ,
        test("nextBefore(routeLeg2.ends) gives routeLeg1.ends"):
          assertTrue:
            routeWithTimes.nextBefore:
              RouteSegment.fromRouteLeg(routeLeg2.ends.getOrElse(???))
            .contains:
              routeLeg1.ends
        ,
        test("nextBefore(routeLeg3.ends) gives routeLeg2.ends"):
          assertTrue:
            routeWithTimes.nextBefore:
              RouteSegment.fromRouteLeg(routeLeg3.ends.getOrElse(???))
            .contains:
