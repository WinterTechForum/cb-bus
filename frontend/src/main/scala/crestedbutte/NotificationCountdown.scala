...
object NotificationCountdown {
  ...
  def startCountdownNotifications($plan: Var[Plan], timeStampsSignal: Signal[WallTime]): Unit = {
    ...
    sendToServiceWorker("START_NOTIFICATIONS", $plan.now()).foreach { response =>
      println(s"Service worker notification started: $response")
    }
  ...
  def stopCountdownNotifications(): Unit = {
    ...
    sendToServiceWorker("STOP_NOTIFICATIONS", null).foreach { response =>
      println(s"Service worker notification stopped: $response")
    }
  }
  
  private def sendToServiceWorker(action: String, plan: Plan): Future[js.Dynamic] = {
    val navigator = dom.window.navigator.asInstanceOf[js.Dynamic]
    
    if (!js.isUndefined(navigator.serviceWorker) && navigator.serviceWorker != null) {
      val serviceWorker = navigator.serviceWorker
      
      serviceWorker.ready.asInstanceOf[js.Promise[js.Dynamic]].toFuture.flatMap { registration =>
        val messageChannel = new dom.MessageChannel()
        
        val responsePromise = scala.concurrent.Promise[js.Dynamic]()
        
        messageChannel.port1.onmessage = (event: dom.MessageEvent) => {
          responsePromise.success(event.data.asInstanceOf[js.Dynamic])
        }
